...
const VideoPlayer = ({ src, variant = "default", className }: VideoPlayerProps) => {
  ...
  const handleMouseEnter = () => {
    if (variant === "masonry" && videoRef.current) {
      videoRef.current.play();
      setIsPlaying(true);
    } else {
  ...
  if (variant === "masonry") {
    return (
      <motion.div
        className={cn(
          "relative w-full h-full rounded-lg overflow-hidden bg-black group",
          className
        )}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}

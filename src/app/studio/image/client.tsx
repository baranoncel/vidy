...
export function StudioImagePageClient() {
      ...
      <header className="mb-6 mt-6 flex items-end justify-between gap-4">
        <div>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-white/70">
            {models.length} image models live
          </span>
          <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Image</h1>
          <p className="mt-2 max-w-xl text-sm text-white/55">Describe the image. Pick a model. Hit ↑.</p>
        </div>
      </header>

      <PageShowcase clips={CATEGORY_REELS.image} />

...
export function StudioVideoPageClient() {
      ...
      <header className="mb-6 mt-6 flex items-end justify-between gap-4">
        <div>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-white/70">
            {models.length} video models live
          </span>
          <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Video</h1>
          <p className="mt-2 max-w-xl text-sm text-white/55">Describe a scene or drop a reference image. Every frontier video model on one bar.</p>
        </div>
      </header>

      {/* Inspiration reel above prompt */}
      <PageShowcase clips={CATEGORY_REELS.video} />
